import 'dart:ui';

class PanColors {
  static const Color white = Color(0XFFFFFFFF);
  static const Color black = Color(0XFF000000);
  static const Color none = Color(0X00000000);
  static const Color font = Color(0XFF414141);
  static const Color text = Color(0xFF454F63);
  static const Color border = Color(0xFFE8E9EA);
  static const Color divider = Color(0xFFEDEEF4);
  static const Color grey1 = Color(0xFFDDDEE4);
  static const Color grey2 = Color(0xFFF7F8F8);
  static const Color grey3 = Color(0xFF8991A2);
  static const Color grey4 = Color(0xFFE2E8ED);
  static const Color splash = Color(0x19000000);
  static const Color highlight = Color(0x19000000);
  static const Color red1 = Color(0xFFD32F2F);
  static const Color green1 = Color(0xFF39AC4C);
}
